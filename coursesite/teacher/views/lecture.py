...
def lectures_page(request, course_id, lesson_id):
    course = Course.objects.get(id=course_id)
    lesson = Lesson.objects.get(lesson_id=lesson_id)
    teacher = Teacher.objects.get(user=request.user)
    try:
        lectures = Lecture.objects.filter(lesson=lesson).order_by('-lecture_num')
...
@login_required(login_url='/landpage')
def lectures_table(request, course_id, lesson_id):
    course = Course.objects.get(id=course_id)
    lesson = Lesson.objects.get(lesson_id=lesson_id)
    teacher = Teacher.objects.get(user=request.user)
...
@login_required(login_url='/landpage')
def lecture_modal(request, course_id, lesson_id):
...
@login_required(login_url='/landpage')
def save_lecture(request, course_id, lesson_id):
    ...
    if request.is_ajax():
        if request.method == 'POST':
            ...
            if lecture_id > 0:
                lecture = Lecture.objects.get(lecture_id=lecture_id)
                form = LectureForm(instance=lecture, data=request.POST, files=request.FILES)
            else:
...
@login_required(login_url='/landpage')
def delete_lecture(request, course_id, lesson_id):
    ...
    if request.is_ajax():
        if request.method == 'POST':
            ...
            try:
                lecture = Lecture.objects.get(lecture_id=lecture_id)
                if lecture.lesson.course.teacher == teacher:
                    lecture.delete()
