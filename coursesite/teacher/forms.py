...
class LessonForm(forms.ModelForm):
    class Meta:
        model = Lesson
        fields = ['lesson_id', 'lesson_num', 'title', 'description']
        labels = {
            'lesson_num': 'Lesson Number',
        }
        widgets = {
            'lesson_num': NumberInput(attrs={'class': u'form-control', 'placeholder': u'Enter Lesson Number'}),
            'title': TextInput(attrs={'class': u'form-control', 'placeholder': u'Enter Title'}),
...
class HomeworkForm(forms.ModelForm):
    class Meta:
        model = Homework
        fields = ['hw_id', 'title', 'description', 'Lesson', 'file', 'answers', 'speaking']
        widgets = {
...
class LectureForm(forms.ModelForm):
    class Meta:
        model = Lecture
        fields = ['lecture_id', 'lecture_num', 'title', 'description', 'youtube_url', 'vimeo_url',
                  'videofile', 'preferred_service']
        labels = {
...
class AssignmentQuestionTypeForm(forms.Form):
    question_num = forms.IntegerField(label='Question #', initial=1, widget=forms.NumberInput(
        attrs={'class': u'form-control', 'placeholder': u'Enter Question Number', 'min': '0', 'max': '100',
               'step': '1'}))
    question_type = forms.CharField(label='Question Type', widget=forms.Select(attrs={'class': u'form-control'},
                                                                               choices=ASSIGNMENT_QUESTION_TYPE_CHOICES))
...
class MultipleChoiceQuestionForm(forms.ModelForm):
    class Meta:
        ...
        fields = ['question_num', 'title', 'description', 'a', 'a_is_correct', 'b', 'b_is_correct', 'c', 'c_is_correct',
                  'd', 'd_is_correct', 'e', 'e_is_correct', 'f', 'f_is_correct', 'marks']
        labels = {
            'question_num': 'Question #',
            'a': 'Option A)',
            'b': 'Option B)',
...
class TrueFalseQuestionForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'false_choice': TextInput(
                attrs={'class': u'form-control', 'placeholder': u'Enter False Choice Description'}),
            'marks': NumberInput(attrs={'class': u'form-control', 'placeholder': u'Enter Marks'}),
        }
